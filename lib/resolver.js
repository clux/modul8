...
Resolver.prototype.locate = function (reqStr, currentPath, currentDomain) {
  ...
  if (domainIgnoreL.indexOf(foundDomain) >= 0) {
    ...
    if (foundDomain === 'npm') {
      ...
      //console.log('trying', expected, order, moduleRoot, currentPath, branch);
      ...
      if (npmMain) {
        ...
        for (var i = 1; i < branch.length; i += 1) {
          ...
          if (!position) {
            error('internal resolver error 1'); // this should never happen
          }
        }
        position.deps[name] = {
          main : npmMain
      ...
      while (true) {
        ...
        if (npmMain) {
          ...
          for (var i = 1; i < oldBranch.length; i += 1) {
            oldPos = oldPos.deps[oldBranch[i]];
            if (!oldPos) {
          ...
          //console.log('found', npmMain, branch, !!branch.length, branch.length);
          if (!branch.length) {
            this.npmTree[name] = {
              main : npmMain
            , deps : {}
          ...
          for (var i = 1; i < branch.length; i += 1) {
            position = position.deps[branch[i]];
            if (!position) {
              error('internal resolver error 2'); // should never happen
            }
          }
