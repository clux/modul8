  ...
  if (domainIgnoreL.indexOf(foundDomain) >= 0) {
    ...
    if (foundDomain === 'npm') {
      ...
      if (this.builtIns.indexOf(name) >= 0) {
        this.npmTree[name] = join(this.builtInDir, name); //err, builtInDir is needed for the analyzer path, right? - shouldnt really TODO
        return [this.npmTree[name], 'npm', true]; // this should be fine, should really be on the npm namespace
      }
      ...
      console.log('trying', expected, order, moduleRoot, currentPath);
      ...
      if (npmMain) {
        ...
        for (var i = 1; i < branch.length; i += 1) {
          ...
          if (!position) {
            error('internal resolver error'); // this should never happen
          }
        }
        position.deps[name] = {
          main : npmMain
