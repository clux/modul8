 ...
 * npmResolve
 *
 * finds the entry point of an npm module
 *
 ...
 */
function npmResolve(absReq, name, silent) {
  var jsonPath = join(absReq, 'package.json')
    , json = {};

  // folder must exists
  if (!path.existsSync(absReq)) {
    if (!silent) {
 ...
 **/
this.locate = function (reqStr, currentPath, currentDomain) {
  ...
  if (domainIgnores.test(reqStr)) {
    ...
    if (foundDomain === 'npm') {
      npmMain = npmResolve(absReq, reqStr, false); // explicitly specified npm:: => throw on errors
    ...
    if (foundDomain && !this.domains[foundDomain]) {
