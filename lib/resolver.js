...
function makeFinder(exts) {
  return function (base, req) {
    for (var i = 0; i < exts.length; i += 1) {
      ...
      //console.log(attempt, '<- attempt, base->', base);
      if (exists(attempt)) {
        return (req + ext);
      }
    }
...
function makeNpmResolver(root) {
  return function (absReq, name) {
    ...
    //console.log('npmResolve', absReq, name, folderPath);
    ...
    //console.log("npmResolve final?", mainFile);
    if (!exists(join(folderPath, mainFile))) {
      error("resolver could not load npm module " + name + "package.json lies about main: " + mainFile + " does not exist");
    }
    //console.log('npmResolve worked, returning', join(absReq, mainFile));
...
function toAbsPath(reqStr, extraPath, domain) {
  //console.log('toAbsPath', reqStr, extraPath);
  if (domainPresent.test(reqStr)) {
    ...
    //console.log("absolute reqStr", reqStr);
    // special case absolutes from npm domain
    // this sandboxes the npm domain (sensible)
    if (domain === 'npm') {
      //console.log('absolute npm string', reqStr, extraPath);
    ...
    //console.log('relative request string', isAbsolute(reqStr));
...
function Resolver(domains, arbiters, exts) {
  ...
  this.exts = exts;
  this.builtInDir = join(__dirname, '..', 'builtins');
}
...
Resolver.prototype.scan = function (absReq, scannable) {
  //console.log("SCAN:", absReq, scannable, this.domains[scannable[0]]);
...
Resolver.prototype.locate = function (reqStr, currentPath, currentDomain) {
  //console.log("resolver locate:", reqStr, 'from', currentPath);
  ...
  //console.log("resolve this ", reqStr, currentDomain, "found: ", foundDomain, absReq);
  if (domainIgnoreL.indexOf(foundDomain) >= 0) {
    ...
    if (foundDomain === 'npm') {
      //console.log('foundDomain is npm')
      if (!this.domains.npm) {
      ...
      if (builtIns.indexOf(name) >= 0) {
        return [join(this.builtInDir, name), 'npm', true]; // this should be fine, should really be on the npm namespace
      }
      // 1. unhandled node base modules
      else if (name === 'path' || name === 'util' || name === 'sys' || name === 'fs') {
        error("cannot require server side node module " + name);
      }

      // 2a. entry point branch to npm domain
      if (currentDomain !== 'npm') {
        var npmMain = this.npmResolve(name, name);
        if (npmMain) {
      ...
      var expected = join(currentPath, 'node_modules', name);

      var npmMain = this.npmResolve(expected, name);
      if (npmMain) {
        //console.log('found npmMain in 2.', npmMain);
        return [npmMain, 'npm', true];
      }
      ...
      while (true) {
        ...
        if (expected.indexOf(this.domains.npm) < 0) { // broken out of domain root
          //console.log('broken out of directory, did not find module');
          break;
        }
        npmMain = this.npmResolve(expected, name);
        if (npmMain) {
          //console.log('found npmMain in 3.', npmMain);
          return [npmMain, 'npm', true];
        }
      }
