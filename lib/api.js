...
Base.prototype.subclassMatches = function (subclass, method) {
  if (this.sub !== subclass) {
    o.logger.warn("Ignoring an invalid call to " + subclass + "::" + method + " after having broken out from the " + subclass + " subclass");
    return false;
  }
  return true;
};
...
Base.prototype.before = function (fn) {
  ...
  if (envCheck()) {
    if (type.isArray(fn)) {
      ...
      o.logger.warn("require 'before' functions to actually be functions - got " + fn);
    }
    else {
      o.pre.push(fn);
    }
...
Base.prototype.after = function (fn) {
  ...
  if (envCheck()) {
    if (type.isArray(fn)) {
      ...
      o.logger.warn("require 'after' functions to actually be functions - got " + fn);
    }
    else {
      o.post.push(fn);
    }
...
Analysis.prototype.hide = function (domain) {
  if (this.subclassMatches('Analysis', 'hide') && envCheck()) {
    if (type.isArray(domain)) {
      ...
      if (domain === 'app') {
        o.logger.warn("cannot ignore the app domain");
      }
      else {
