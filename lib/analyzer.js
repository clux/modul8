...
function Analyzer(obj, before, compile) {
  ...
  this.detective = deputy(join(__dirname, '..', 'deputy.json'));
...
Analyzer.prototype.dependencies = function (absReq, extraPath, dom) {
  ...
  // detective scanning each file is the most time consuming process on big codebases so we cache
  //console.log('DETECTIVE TIME');
  //var deps = this.detective.find('require("a"); require("b")');
  return this.detective(code).map(function (req) { //TODO use caching layer, detective is slow..
    //console.log('RESOLVE');
    var a = resolve(req, extraPath, dom);
    return a;
  });
};
...
Analyzer.prototype.print = function (extSuffix, domPrefix) {
  ...
  var objCount = function (o) {
    return Object.keys(o).filter(function (e) {
      return ignores.indexOf(o[e].domain) < 0;
    }).length;
  };
  ...
  var print = function (branch, level, parentAry) {
    ...
    Object.keys(branch.deps).forEach(function (uid) {
      ...
      if (ignores.indexOf(depObj.domain) >= 0) {
        return;
      }
      idx += 1;

      var hasChildren = objCount(depObj.deps) > 0
